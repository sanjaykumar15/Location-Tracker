...
class OperationModel : ViewModel() {
    ...
    val config = RealmConfiguration.create(schema = setOf(CredentialObject::class,LocationObject::class))
    var realm = Realm.open(config)

    fun signUp(nameVal: String, emailVal: String, passVal: String, contactVal: String){

        val resultItems: RealmResults<CredentialObject> = realm.query<CredentialObject>().sort("id",Sort.DESCENDING).limit(1).find()
    ...
    fun logIn(email: String, pass: String){
        ...
        val res: RealmResults<CredentialObject> = realm.query<CredentialObject>("email == $0 AND pass == $1",email,pass).find()
        logInResultData.value = realm.copyFromRealm(res)
        /*val result = realm.where(CredentialModel::class.java)
            .findAll()
        logInResultData.value = realm.copyFromRealm(result)*/

    }

    fun writeLocation(id: Int, cityVal: String, stateVal: String, pinVal: String,
    ...
    fun getLocationFromDB(id: Int){
        ...
        val res: RealmResults<LocationObject> = realm.query<LocationObject>("userId == $0",id).sort("dateTime", sortOrder = Sort.DESCENDING).find()
        locationResult.value = realm.copyFromRealm(res)

    }

    fun getLoggedInUser(){
        val resultItems: RealmResults<CredentialObject> = realm.query<CredentialObject>("isLoggedIn == $0",true).find()
        loggedUserResultData.value = realm.copyFromRealm(resultItems)
    }

    fun updateIsLoggedIn(id: Int, isLoggedInVal: Boolean){
        realm.writeBlocking {
            val res: RealmResults<CredentialObject> = this.query<CredentialObject>("id == $0",id).find()
